...
```
## 绝对路径使用perf
sudo /usr/lib/linux-tools/5.15.0-164-generic/perf record  -g -- ./your_app
会生成 perf.data 文件

## 使用 perf script导出数据
sudo /usr/lib/linux-tools/5.15.0-164-generic/perf script > perf.script
...
# 简化命令
## 生成 perf.data
echo "alias perf-wsl='sudo /usr/lib/linux-tools/5.15.0-164-generic/perf'" >> ~/.bashrc
source ~/.bashrc
